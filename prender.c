...
int main()
{
    ...
    while(1)
    {
        ...
        if(falling)
        {
            ...
            float player_sector = sectors[player.sector].floor;
            if(player.velocity.z < 0 && nextz  < player_sector + eyeheight) // When going down seg faulting 
            {
                /* Fix to ground */
        ...
        player.yaw   = -1 * yaw - player.velocity.z*0.5f;
        MovePlayer(0,0);
